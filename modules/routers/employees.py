...
async def get_all_employees(page: int = 0, items: int = 20) -> tp.Dict[str, tp.Any]:
    """
...
@router.post("/api/v1/employees", dependencies=[Depends(check_user_permissions)])
async def create_new_employee(employee_data: Employee) -> None:
    """Endpoint to create new employee"""
    await MongoDbWrapper().add_employee(employee_data)


@router.delete("/api/v1/employees/{rfid_card_id}", dependencies=[Depends(check_user_permissions)])
async def delete_employee(rfid_card_id: str) -> None:
...
async def get_employee_by_card_id(rfid_card_id: str) -> tp.Optional[Employee]:
